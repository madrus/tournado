...
export function ErrorMessage({
  ...
  const getErrorStyling = (color: string, variantType: string): string => {
    const baseClasses = 'mt-2 px-3 py-2 rounded-md text-sm font-medium'

    if (variantType === 'inline') {
  ...
  return (
    ...
    >
      ...
      <ErrorIcon className='mt-0.5 h-4 w-4 flex-shrink-0' size={16} />
      <span>{children}</span>
    </div>
  )
}
