 ...
 */
export async function loader({ request }: Route.LoaderArgs): Promise<Record<string, never>> {
	// Only allow users with roles other than PUBLIC to access admin panel
	const allowedRoles = [Role.ADMIN, Role.MANAGER, Role.EDITOR, Role.BILLING, Role.REFEREE]
