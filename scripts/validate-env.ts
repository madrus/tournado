...
function formatValue(envVar: EnvVar): string {
  const value = process.env[envVar.name]
...
function printReport(context: EnvContext, result: ValidationResult): void {
  console.log('\n' + '='.repeat(80))
  console.log(`Environment Validation Report - Context: ${context.toUpperCase()}`)
  ...
  if (result.configured.length > 0) {
    ...
    console.log('-'.repeat(80))
    for (const envVar of result.configured) {
      console.log(`  ${envVar.name}`)
      console.log(`     Value: ${formatValue(envVar)}`)
  ...
  if (result.valid) {
    ...
    console.log(
      `❌ Validation failed: ${result.missing.length} missing, ${result.empty.length} empty`
    )
    console.log('\nTo fix:')
    console.log('  1. Check the variable names above')
    console.log('  2. Add them to your .env file (local) or GitHub Secrets (CI)')
...
function main(): void {
  ...
  if (!validContexts.includes(context)) {
    ...
    console.error(`Valid contexts: ${validContexts.join(', ')}`)
    process.exit(1)
  }
