...
vi.mock('~/components/buttons', () => ({
  ActionLinkButton: ({
    ...
    <a
      href={to}
      className={`inline-flex items-center justify-center rounded-lg border bg-primary-600 font-semibold text-white ${className || ''}`}
      data-testid={dataTestId}
    >
      {label || children}
    </a>
  ...
  describe('View Teams Button Routing', () => {
    ...
    test('should route to admin teams page for users with admin panel access', () => {
      ...
      adminPanelRoles.forEach(role => {
        ...
        const viewTeamsButton = screen.getByTestId('view-teams-button')
        expect(viewTeamsButton).toHaveAttribute('href', adminPath('/teams'))
  ...
  describe('Consistency with AppBar', () => {
    test('should use same routing logic as AppBar Teams menu item', () => {
      ...
      const roles: Array<{
        ...
        { role: 'MANAGER', expectedRoute: adminPath('/teams') },
        { role: 'REFEREE', expectedRoute: adminPath('/teams') },
        { role: 'ADMIN', expectedRoute: adminPath('/teams') },
      ]
